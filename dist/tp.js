"use strict";
let get_notification_block = document.querySelector('.text_notification');
...
let getShopVariants = Array.from(document.getElementsByClassName('dropdown-item'));
...
function LoadingScreen() {
    let getLoaderBlock = document.querySelector('.loader');
...
function SetCurrentShopIcon(shop_index) {
    let shopPersonChooseBlock = document.querySelector('.shop_user_choose');
...
function GeneratingShopInf(shopDataAPI) {
    ...
    function GenProducts(products_to_gen, json, mass_of_chooseBtns) {
        if (products_to_gen + 5 > json.length && json.length > products_to_gen) {
            for (let i = products_to_gen; i < json.length; i++) {
                let getObj = json[i];
                CardCreation(getObj['img'], getObj['product'], getObj['price']);
            }
        }
        ...
        mass_of_chooseBtns = Array.from(document.getElementsByClassName('btn-primary'));
    ...
    function AddingProduct(start_size, json, mass_of_chooseBtns) {
        window.onscroll = () => {
